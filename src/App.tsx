...
function App() {
    ...
    const [isLoading, setIsLoading] = useState({ ocr: false, aiData: false, generating: false, detecting: false });
    ...
    const handleTemplateUpload = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        const type = file.type.startsWith('image/') ? 'image' : (file.type === 'application/pdf' ? 'pdf' : null); if (!type) { alert("Unsupported file type."); return; }
        let data;
        if (type === 'pdf') {
            try {
            ...
            } catch (e) { alert("Could not process PDF."); return; }
        } else {
    ...
    const handleUpdatePlaceholder = (id: string, updates: Partial<Placeholder>) => { setPlaceholders(placeholders.map(p => p.id === id ? { ...p, ...updates } : p)); };
