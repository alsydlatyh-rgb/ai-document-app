  ...
  try {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      if (template.type === 'pdf') {
        ...
        const image = await (template.file.type === 'image/png' ? pdfDoc.embedPng(templateBytes) : pdfDoc.embedJpg(templateBytes));
      ...
      for (const placeholder of placeholders) {
        ...
        // FINAL CORRECTED Y-COORDINATE: Aligns the baseline of the first line just below the top of the box.
        let y = pageHeight - placeholder.y - placeholder.fontSize;

        for (const line of lines) {
          page.drawText(line, { x: placeholder.x, y, font, size: placeholder.fontSize, color: rgb(r, g, b), lineHeight: placeholder.fontSize * 1.2 });
          y -= placeholder.fontSize * 1.2; // Move down for the next line
        }
      }
